...
async function main() {
    console.log("Starting GoHedgePreProduction contract deployment...");
    ...
    // Use a valid Fuji AVAX/USD price feed address for testnet deployment
    const AVAX_PRICE_FEED = "0x0A77230d17318075983913bC2145DB16C7366156"; // Fuji AVAX/USD
    const CCIP_ROUTER = "0x88E492127709447A5ABEFdaB8788a15B4567589E"; // Fuji CCIP router

    // Deploy GoHedgePreProduction contract
    console.log("\nDeploying GoHedgePreProduction contract...");
    const GoHedgePreProduction = await ethers.getContractFactory("GoHedgePreProduction");
    const goHedge = await GoHedgePreProduction.deploy(
        AVAX_PRICE_FEED,
        usdcAddress,
        CCIP_ROUTER
    );
    await goHedge.waitForDeployment();
    const contractAddress = await goHedge.getAddress();

    console.log("GoHedgePreProduction deployed successfully!");
    ...
    const activeContractsCount = await goHedge.activeContractsCount();
    const usdcTokenAddress = await goHedge.getUSDCAddress();
    ...
    const avaxPrice = await goHedge.testPrices("AVAX");
    const btcPrice = await goHedge.testPrices("BTC");
    const ethPrice = await goHedge.testPrices("ETH");
    const usdcPrice = await goHedge.testPrices("USDC");
    ...
    const [totalContracts, activeContracts, triggeredContracts, lastCheck, enabled] =
        await goHedge.getAutomationStats();
    ...
    const deploymentInfo = {
        ...
        contractType: "GoHedgePreProduction",
        features: [
            "USDC Support",
            "Time-based Automation",
            "Dual Reserve System",
            "CCIP Cross-Chain Whitelist",
            "Test Mode Enabled"
        ],
        testPrices: {
    ...
    console.log("\nDeployment information saved to deployment-gohedge-info.json");
    ...
    console.log("   - GoHedgePreProduction:", contractAddress);
    console.log("   - MockUSDC Token:", usdcAddress);

    console.log("\n2. To create AVAX reserve contract:");
    console.log(`   await contract.createContract("AVAX", ${ethers.parseUnits("18", 8)}, startDate, endDate, false, ethers.parseEther("1"), ethers.parseEther("0.01"), true, false, { value: ethers.parseEther("1") })`);

    console.log("\n3. To create USDC reserve contract:");
    console.log("   - First approve: await usdcToken.approve(contractAddress, amount)");
    console.log(`   - Then create: await contract.createContract("AVAX", ${ethers.parseUnits("18", 8)}, startDate, endDate, true, ethers.parseUnits("1000", 6), ethers.parseEther("0.01"), true, false)`);
...
main()
    .then((addresses) => {
        ...
        console.log("=".repeat(60));
        console.log("GoHedgePreProduction address:", addresses.contractAddress);
        console.log("MockUSDC address:", addresses.usdcAddress);
        console.log("\nReady for testing with dual AVAX/USDC reserves and CCIP features!");
        process.exit(0);
    })
    .catch((error) => {
